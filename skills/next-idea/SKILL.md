...
- **Artifact-first gate (default product lifecycle).** Under `initiatives/[Initiative]/[Project]/[Idea]/`, walk stages in the order defined in `IDEA_LIFECYCLE.md` at the repo root. Honor project overrides from that idea’s `00-how-to-use.md` or `PROCESS.md` when present. Honor a **Waiver: skip Pressure Test** line in that idea’s **Notes** in `ideas.md` (see IDEA_LIFECYCLE.md).
  - **Brief:** `01_brief.md`
