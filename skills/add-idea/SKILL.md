...
3. **Idea title** - short, noun-style name for the **Idea** column. It must match the eventual `initiatives/[Initiative]/[Project Name]/[Idea Name]/` folder name when lifecycle artifacts exist.
...
6. **Richness** - one line only, or extra detail (paragraphs, lists, bullets). Rich content goes in `initiatives/[Initiative]/[Project Name]/[Idea Name]/01_brief.md`, not only in the table.
...
Do **not** add rows to **Awaiting your approval** for a simple backlog capture. Create `initiatives/[Initiative]/[Project Name]/[Idea Name]/` only when you need a brief or richer scoped notes.
...
- **Notes / next action** - For a sentence-only idea, a single line is enough. For rich ideas, one line plus a link to the brief, e.g. `[brief](<Project>/<Idea>/01_brief.md)` relative to that initiative (same path shape as on disk under `initiatives/[Initiative]/`).
...
Create only when `initiatives/[Initiative]/[Project Name]/` is new. Keep it short:

- State that all idea work for this project lives under `initiatives/[Initiative]/[Project Name]/[Idea Name]/`.
- Link to the initiative’s `ideas.md` at `../ideas.md`, and to `SYSTEM_OVERVIEW.md` / `IDEA_LIFECYCLE.md` at repo root as `../../../SYSTEM_OVERVIEW.md` (the file lives at `initiatives/[Initiative]/[Project Name]/00-how-to-use.md`, so three levels up to the repo root).
...
- **General** project: use `General/[Idea Name]/` under the initiative for new briefs.
